...
async function processAccount(browserContext, accountUrl, accountNumber, proxy) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            try {
                await page.waitForSelector(clickskipcheckinButtonSelector, { timeout: 2000 });
                const skipcheckinButton = await page.$(clickskipcheckinButtonSelector);
                if (skipcheckinButton) {
                    await skipcheckinButton.click();
                    console.log(``);
                } else {
                }
            } catch (err) {
